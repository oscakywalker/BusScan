...
cv_splits = [
    (0, 200),   # 第一组测试集：1-200
    (200, 400), # 第二组测试集：201-400
    (400, 600), # 第三组测试集：401-600
    (600, 800), # 第四组测试集：601-800
    (800, len(df)) # 第五组测试集：801-最后
]
...
def cross_validation_mse(df, cv_splits, time_threshold):
    ...
    for start, end in cv_splits:
        ...
        # 准备测试数据
        test_state_0 = test_data[(test_data['State'] == 0) & (test_data['Lasting Time'] <= time_threshold)]
        test_durations_state_0 = test_state_0['Lasting Time']
        test_events_state_0 = [1] * len(test_state_0)

        test_state_1 = test_data[(test_data['State'] == 1) & (test_data['Lasting Time'] <= time_threshold)]
        test_durations_state_1 = test_state_1['Lasting Time']
